...
async function bugTwo(coinName, bbfix, fix) {
  try {
    ...
    let MarketBuy = await binance.futuresMarketBuy(coinName, hedgeAmt, {
      positionSide: "LONG",
    });
    let posAmt = amt;
    ...
    for (let market of markets) {
      ...
      if (obj.positionSide == "SHORT" && obj.symbol == coinName) {
        ...
        let limitPrice = (obj.entryPrice * 0.995).toFixed(fix);
      ...
      if (obj.positionSide == "LONG" && obj.symbol == coinName) {
        ...
        let limitPrice = (obj.entryPrice * 1.005).toFixed(fix);
    ...
    while (true) {
      ...
      if (plusAmt === 0 && minusAmt === hedgeAmt * 1 && !change) {
        ...
        let limitBuy = await binance.futuresMarketBuy(coinName, allAmt, {
          positionSide: "LONG",
          type: "STOP_MARKET",
          stopPrice: (longEntryPrice * 1.01).toFixed(fix),
        });
        let limitSell = await binance.futuresBuy(
          coinName,
          minusAmt,
        ...
        let MarketSell = await binance.futuresMarketBuy(coinName, minusAmt, {
          positionSide: "SHORT",
          type: "STOP_MARKET",
          stopPrice: (shortEntryPrice * 1.01).toFixed(fix),
        });
        dir = "LONG";
        change = true;
      }
